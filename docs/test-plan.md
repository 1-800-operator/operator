...
- **A — Config loader** (session 133, 2026-04-19) — 6 tests added to `tests/test_config_loader.py`, all pass. Covers missing/unknown `OPERATOR_BOT`, yaml field parse + defaults, `SYSTEM_PROMPT` composition (single `system_prompt` field — collapsed in session 178; pre-178 docs/tests referenced separate `personality`/`ground_rules` blocks), `intro_on_join` default-True, and MCP server filter/`tool_timeout_seconds`/`${VAR}` env resolution. Loader tests use a tmp `config.py` copy + tmp `agents/<bot>/config.yaml` so no real agents are touched.
