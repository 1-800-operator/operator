...
- [x] `connectors/attach_adapter.py` (959 → 957 LOC) — DONE in commit `8dc8b62` (S201, connector cell A — bundled with session.py promotion). Promoted `_is_real_meet_room` + `_MEET_ROOM_RE` from `macos_adapter` into `session.py`; the cross-adapter import was a self-acknowledged "temporary smell" comment in code, and attach_adapter being the second adapter using it was the threshold the comment named for promotion. Other cleanups: dropped chmod try/except around the debug-screenshot fallback in `_ensure_chat_open` (same trust-the-OS pattern); added belt-and-suspenders `os.chmod(DIAL_PROFILE_DIR, 0o700)` after `makedirs` since `mode=` doesn't tighten existing dirs (Google session cookies in dial profile shouldn't be world-readable on shared hosts); wrapped `stderr_sink = open("/tmp/operator.log", "ab")` in a `with` block in `_start_audio_pipeline` so the parent fd closes after `spawn_disclaimed`'s dup2 (was leaking a fd for the connector lifetime); tightened `_cdp_belongs_to_dial` from a plain `DIAL_PROFILE_DIR in stdout` substring match to `f"--user-data-dir={DIAL_PROFILE_DIR}" in stdout` to eliminate false-positive against sibling profile paths like `~/.operator/dial_profile_backup`.
...
- [x] `__main__.py` (709 → 696 LOC) — DONE in commit `bbae03f` (S202). Two surgical removals: (1) six dead `logging.getLogger("openai"/"anthropic").setLevel(...)` lines across `_run_dial`/`_run_macos`/`_run_linux` — both libraries deleted in 14.19.7's provider purge, zero imports anywhere in src/. (2) Redundant inner `if name != "claude"` guard at top of `_run_dial` — main() dispatcher already filters at argv parse, function is module-private with one caller, the inner check never fires. Out of scope: three near-identical 7-line logging-setup blocks could fold into a helper (cross-function refactor, low payoff today); `runner._stop_event.is_set()` private-attr access at lines 619+698 (mild PEP8 — ChatRunner audit territory).
...
- [x] `pipeline/install_preflight.py` (175 LOC) — REVIEW-only in S203 (no commit; product call open). Module is a phantom feature: `run_install_preflight()` is the docstring-promised entry for "the top of `operator setup`" but no `operator setup` subcommand exists in `__main__.py`. The `chromium_installed` helper is genuinely live (called by `doctor.py`) so it stays regardless. Decision needed: (a) add `operator setup` subcommand wired to the orchestrator, (b) auto-invoke at top of `dial`/`dial`/`login` (silent when ready), (c) document `curl|sh` as the only path + delete orphans, (d) keep as scaffolding. Logged in revisit log below.
- [x] `pipeline/doctor.py` (282 LOC) — DONE in commit `1bf4527` (S203). Two doc-staleness fixes (no behavior change): (1) `_check_chromium()` had a local `import sys` already imported at module top — redundant. (2) Module docstring claimed "Phase 14.20 will add a Screen & System Audio Recording check when dial caption capture ships" — those TCC checks (Screen Recording + Microphone) are already live at lines 182-238 (landed in 14.20.4). Refreshed to today's reality. Also fixed `CheckResult` field list in the docstring — was `(name, status, fix)` but the dataclass has four fields (`name, ok, detail, fix`). Subprocess calls (`git --version`, `helper --probe`) verified safe (trusted paths, list args, no shell, no injection); `_check_microphone`'s "see Screen Recording check" detail with empty fix correctly suppresses duplicate fix-line printing in `run_doctor`.
...
- **A1 (security): DIAL_PROFILE_DIR perms not tightened on existing dirs.** ✅ Fixed: added `os.chmod(DIAL_PROFILE_DIR, 0o700)` after `makedirs` (mirrors `macos_adapter._browser_session`'s belt-and-suspenders pattern). `mode=` only fires at creation; if the dir already existed with looser perms (umask-default 0o755) Google session cookies were silently world-readable on shared hosts.
...
- **B1 (edge case): `_cdp_belongs_to_dial` substring false-positive.** ✅ Fixed: tightened from plain `DIAL_PROFILE_DIR in stdout` to `f"--user-data-dir={DIAL_PROFILE_DIR}" in stdout`. Prevents silent attach-to-wrong-Chrome when a sibling profile (e.g. `dial_profile_backup`) has overlapping path text.
...
- [x] ~~Verify `playwright install chromium` completes (~170 MB)~~ — **obsolete S246**: bundled Chromium dropped in 14.22.5 (dial CDP-attaches to real Chrome.app)
...
- [ ] Run `/operator:dial <meet-url>` end-to-end on the fresh machine — bot joins, `@claude` reaches inner-claude, reply lands in chat
...
- [x] Confirm `~/.operator/dial_profile/` (dial Chrome user-data-dir with Google session cookies) is never copied into debug dumps — verified: not referenced by `save_debug`; dir at 0700
...
- Mitigation by mode:
  - `/operator:dial` (default) + `/operator:dial-strict`: PreToolUse permreq pops before Read/Bash/Write → user explicitly approves each access.
  - `/operator:dial-yolo`: no permreq, tools run unattended. User opts in; SKILL.md should warn.
...
3. "dial-yolo runs all tools unattended; don't use in meetings where you don't fully trust claude with arbitrary tool access."
