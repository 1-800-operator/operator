...
- [ ] audio-helper Swift binary (sources tracked separately; deferred to 14.20.5 pre-notarize review — Apple Dev cert still in flight)
...
- **`install_preflight.run_install_preflight` + `readiness.preflight_mcp_readiness` orphans (S203 REVIEW-only).** Both are phantom features: `run_install_preflight` is the docstring-promised entry for "the top of `operator setup`" but no `operator setup` subcommand exists in `__main__.py`; `preflight_mcp_readiness` + `report_mcp_readiness` claim to run "inside `_run_bot()` after `OPERATOR_BOT` is set" but `_run_bot` doesn't wire them in. ~600 LOC of test coverage of unreachable code (`test_1574_readiness.py`, `test_15745_preflight.py`). **Latent landmine**: `preflight_mcp_readiness` lazy-imports `from _1_800_operator.pipeline.auth import run_auth` at line 245 — `pipeline/auth.py` doesn't exist; tests pass a mock so the import never fires. References to `operator auth <name>` at lines 167+288 also point at a non-existent CLI subcommand. Decision needed: (a) build `operator setup` subcommand + `operator auth` subcommand + `pipeline/auth.py` to wire the orphans into runtime; (b) auto-invoke install preflight at top of `dial`/`slip`/`login` (silent when ready); (c) document `curl|sh` as the only path + delete orphans + delete ~600 LOC of test coverage; (d) keep as scaffolding for a future audit pass. The `chromium_installed` helper in `install_preflight` is genuinely live (called by `doctor.py`) and stays in any outcome.
- **`openai==2.29.0` + `anthropic==0.94.0` Python deps (S203 deferred).** Zero src/ imports post the 14.19.7 single-provider claude_cli pivot. Same product call gates them as the `_tail_messages` / `_build_messages` chain in `llm.py`: if a second provider is in v1 scope the deps stay, if dropped they go in the same commit as the llm.py vestigial cleanup.
