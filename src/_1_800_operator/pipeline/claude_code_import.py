...
def _wrap_http_as_stdio(entry: dict, transport: str) -> dict:
    ...
    gate (readiness.oauth_cache_exists) and the `operator auth <name>`
...
def _stdio_block_from_entry(entry: dict) -> dict:
    """Convert a Claude Code stdio MCP entry to the Operator config shape."""
    block = {
        "enabled": True,
...
def discover_hosted_mcps_via_cli() -> list[ImportedMCP]:
    ...
    Operator's own `operator auth <name>` flow regardless. If Claude
    Code says "Connected", that's a claude.ai-side token; Operator
...
def append_env_placeholders(var_names: Iterable[str], env_file: Path) -> list[str]:
    ...
    with env_file.open("a", encoding="utf-8") as f:
        if needs_leading_nl:
            f.write("\n")
        f.write("\n# Added by operator — claude-agent MCP import\n")
