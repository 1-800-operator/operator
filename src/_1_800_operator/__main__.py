...
Dial mode entry point. CDP-attaches `claude` to a dedicated Chrome window
running the meeting.

Usage:
    operator dial claude <url>    Attach claude to a dial Chrome session
...
# Module-load timestamp — anchors the `TIMING dial_startup` line so we can
...
# Bot names operator's dial dispatch accepts as the first positional after
# `dial`. v1 ships claude only; a future codex bridge would add "codex"
...
# user (or, more often, a model improvising) invokes `operator dial <url>`
...
# meet URL (typos like `operator dial cluade <url>` still error cleanly).
...
# other tabs they had open in dial Chrome.
#
...
# Playwright didn't launch Chrome), and dial Chrome keeps running so
...
_DIAL_LOCK_PATH = Path.home() / ".operator" / "dial.pid"
...
def _preflight_audio_helper_tcc() -> None:
    ...
    **Why `open -W -n -a` for the warmup (not `_disclaimed_spawn`):** both
    ...
    # function exit so the dial startup trace can subtract this cost.
    ...
    if sr == "ok" and mic == "ok":
        ...
        # _run_dial) to keep the LS DB tidy without blocking the user.
    ...
    if denied:
        ...
        for pane, _ in denied:
            print(f"        - {pane}")
        print()
        print("  Dial will continue in chat-only mode until you re-enable.")
...
def _pid_is_operator(pid: int) -> bool:
    """True iff <pid> is a live operator-dial process.
    ...
    cmd = r.stdout
    return "_1_800_operator" in cmd or "operator dial" in cmd


def _acquire_dial_lock():
    """Take the dial singleton lock at ~/.operator/dial.pid.

    Returns None on success (lock now held by this process). Returns
    the live PID of an existing operator-dial daemon when one is
    ...
    a daemon launched via `operator dial <url>` (no positional). See
    S219 investigation.
    """
    _DIAL_LOCK_PATH.parent.mkdir(parents=True, exist_ok=True)
    while True:
        try:
            fd = os.open(
                str(_DIAL_LOCK_PATH),
            ...
            try:
                other_pid = int(_DIAL_LOCK_PATH.read_text(encoding="utf-8").strip())
            except (OSError, ValueError):
                try:
                    _DIAL_LOCK_PATH.unlink()
                except OSError:
                    pass
                continue
...
def _write_dial_lock(pid: int) -> None:
    """Overwrite the dial lockfile with <pid>. Called by the daemonize
    parent after fork so the lockfile points at the surviving child."""
    try:
        _DIAL_LOCK_PATH.write_text(f"{pid}\n", encoding="utf-8")
    except OSError:
        pass


def _release_dial_lock() -> None:
    """Delete the dial lockfile iff this process owns it. Idempotent.
    ...
    """
    try:
        owner = int(_DIAL_LOCK_PATH.read_text(encoding="utf-8").strip())
    except (OSError, ValueError):
        return
    if owner != os.getpid():
        return
...
def _print_usage():
    print("Usage:")
    print("  operator dial claude <url>          Dial — guarded by default; permission asks bridged to meeting chat")
    print("  operator dial-strict claude <url>   Dial — guarded, every prompt requires @claude (no sticky window)")
    print("  operator dial-yolo claude <url>     Dial — no permission asks, every chat message goes to claude")
    ...
    print("  operator hangup                     Gracefully disconnect the running dial session")
    print("  operator doctor                     Diagnostic check — is the world ready?")
    print()
    print("Flags:")
    print("  --resume-session <id>               Bridge an existing Claude Code session into dial")
...
def main():
    ...
    if first in ("dial", "dial-strict", "dial-yolo"):
        # Default dial is now guarded (formerly /operator:dial-guarded
        # behavior); dial-strict is guarded + requires @claude every
        # turn; dial-yolo is unattended with no permission asks. The
        # pre-S238 `dial-guarded` name is gone — pre-launch hard cutover.
        mode = {"dial": "dial", "dial-strict": "strict", "dial-yolo": "yolo"}[first]
        if len(argv) < 2:
        ...
        # drops the `claude` positional and invokes `operator dial <url>`
        ...
        # the user/model supplied a bot. Typos like `dial cluade <url>`
        # fall through to the "Unknown bot" error because `cluade` doesn't
        # look URL-shaped. Same inference applies to dial-guarded.
        ...
        return _run_dial(name, rest, mode=mode)
...
def _daemonize_and_announce(url):
    ...
    up to exit. When the model fires `operator dial claude <url> &` the
    ...
    users (someone running `operator dial claude <url>` in iTerm)
    ...
    pid = _os.fork()
    if pid > 0:
        # Parent — rewrite the dial lockfile so it points at the
...
def _infer_bot_from_surface():
    ...
    Walks SURFACE_BOTS in order and returns the first match. The dial
    ...
    (`operator dial <url>` without `claude`). When more than one surface
...
def _read_current_meeting_url():
    """Return the meet URL of the currently-active dial session, or None.
...
def _run_status():
    ...
    Reads the ~/.operator/.current_meeting marker written by _run_dial at
    ...
    most recent failure (if any) so a user whose last dial silently failed
...
def _print_last_failure_hint():
    """Append a one-line hint about the last dial failure, if recent.
    ...
    successful dial start, but if the user never retried, anything beyond
    ...
    print(f"last dial failed: {msg}{suffix}")


def _run_hangup():
    """Send SIGTERM to the running operator-dial daemon, if any.

    Source of truth is the dial lockfile (~/.operator/dial.pid) written
    ...
    The dial process's signal handler does the graceful teardown:
    ...
    pid = None
    if _DIAL_LOCK_PATH.exists():
        try:
            pid = int(_DIAL_LOCK_PATH.read_text(encoding="utf-8").strip())
        except (OSError, ValueError):
            pid = None
        if pid is not None and not _pid_is_operator(pid):
            # Stale — reclaim so subsequent dial attempts succeed.
            try:
                _DIAL_LOCK_PATH.unlink()
            except OSError:
                pass
    ...
    # Poll on the dial lockfile being released, not on the daemon's pid
    # exiting. The daemon's _shutdown releases the lock early (~500ms
    # after SIGTERM — intentional design so /operator:dial can
    ...
    # follow-up /operator:dial would otherwise race on.
    deadline = _time.monotonic() + 3.0
    while _time.monotonic() < deadline:
        if not _DIAL_LOCK_PATH.exists():
            break
...
def _run_dial(name, rest, *, mode="dial"):
    """dial mode — launch a dedicated Chrome window for the meeting and
    CDP-attach claude to it.

    Dial Chrome lives at ~/.operator/dial_profile/ — operator-owned,
    separate from the user's main browser. First run: user signs into
    Google in dial Chrome once, cookies persist for future sessions.
    ...
    `mode` is one of:
      - "dial"   default. Guarded — operator intercepts permission asks
    ...
    _t_dial_entry = _startup_time.monotonic()
    guarded = mode in ("dial", "strict")
    ...
    if not url:
        print(
            "dial requires a Meet URL: operator dial claude <https://meet.google.com/xxx-xxxx-xxx>"
        )
        return 0
    ...
    if sys.platform != "darwin":
        print("dial mode is currently macOS-only.")
        return 0

    # Singleton guard — refuse to start if another operator dial is already
    # running. Without this, the desktop app can stack operators on the same
    # dial Chrome (e.g. when the model retries a failed dispatch), each
    ...
    # ("operator dial is already running…") regardless of exit code. The
    ...
    if other_pid is not None:
        ...
        if existing_url:
            print(
                f"operator dial is already running — already in {existing_url}. "
                f"Use /operator:hangup to leave that meeting first, then retry."
            )
        else:
            print(
                f"operator dial is already running (pid {other_pid}). "
                f"Use /operator:hangup to leave that meeting first, then retry."
            )
        return 0
    ...
    # continues if user denies — degraded dial (silent captions) beats
    ...
    #
    # Clear last_failure.json — a new dial is the user's "try again";
    # doctor's "last meeting failure" section means "since you started
    # the most recent dial", not "ever". A fresh failure during this
    ...
    _log_pre.info(
        f"TIMING dial_startup "
        f"python_boot_ms={int((_t_dial_entry - _T_MODULE_LOAD) * 1000)} "
        f"argv_ms={int((_t_argv_done - _t_dial_entry) * 1000)} "
    ...
    # Two-tier resume-session resolution (see comment block at top of _run_dial).
    resume_source = None
    ...
    if guarded:
        ...
        log.info("dial: guarded mode — classifier sidecar spawning in parallel")

    llm = LLMClient(provider)
    llm.set_record(meeting_record)
    if resume_session_id:
        log.info(
            f"dial: bridging existing Claude Code session {resume_session_id} "
            f"into meeting (source={resume_source})"
        )
    else:
        log.info("dial: no resume session — starting fresh")
    ...
    connector = AttachAdapter(
        reply_prefix=claude_bridge.REPLY_PREFIX_DIAL,
        jsonl_path=meeting_record.path,
    )

    ui.say("Launching dial Chrome…")
    try:
        connector.join(url)
    except DialAttachError as e:
        ui.err(str(e))
    ...
    def _shutdown(signum=None, frame=None):
        ...
        #   dial.pid. These are pure lookup signals — nothing in the rest of
        #   the teardown reads them, so releasing first means subsequent
        #   /operator:status and /operator:dial see truth immediately (the
    ...
    try:
        log.info(f"Starting Operator dial mode — attached to {url}")
        runner.run(url)
    except KeyboardInterrupt:
        log.info("Interrupted — detaching")
    finally:
        _shutdown()
        ui.ok("Detached — dial Chrome stays open so the meeting can continue. Goodbye.")
    return 0
...
def _run_wiretap(url):
    ...
    Attaches to dial Chrome, joins the meeting, captures chat + captions
    ...
    # matter. Same warmup path as dial.
    _preflight_audio_helper_tcc()
    ...
    ui.say("Launching dial Chrome for wiretap…")
    try:
        connector.join(url)
    except DialAttachError as e:
        ui.err(str(e))
    ...
    def _shutdown(signum=None, frame=None):
        ...
        if signum:
            log.info(f"Received signal {signum} — shutting down")
        # Same parallel-teardown structure as dial mode (see dial _shutdown
        ...
        # closes its stdin. See dial _shutdown for the rationale.
        attended = sorted(getattr(runner, "_attended_participants", set()))
        self_name = getattr(runner, "_last_self_name", "") or ""
        # See dial _shutdown for the rationale on _audio_worker_pid as
